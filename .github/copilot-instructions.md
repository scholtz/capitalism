...
12. **After running frontend lint with `--fix`, re-open `projects/frontend/e2e/exchange.spec.ts` and verify any `getAttribute()` assertions were not rewritten into locator-only checks.** Then rerun at least `CI=true npx playwright test --project=chromium e2e/exchange.spec.ts` before pushing.
